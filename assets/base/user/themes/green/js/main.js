    ...
    ********************************/
    ...
    $(document).click(function(e) {
        ...
        if ( $('.midrub-planner').length > 0 ) {
            ...
            if ( (!$('.midrub-planner').is(e.target) && $('.midrub-planner').has(e.target).length === 0) && (!$( '.open-midrub-planner' ).is(e.target) && $('.open-midrub-planner').has(e.target).length === 0) ) {
     ...
     * @param object e with global object
     */
    $(document).on('click', '.container-fluid nav.navbar ul li > a', function (e) {
        ...
        if ( $(this).closest('li').find('ul').length > 0 ) {
            ...
            if ( !$(e.target).closest('li').hasClass('nav-active') ) {
                ...
                // Hide the submenus
                $('.container-fluid nav.navbar ul li').removeClass('nav-active');
                $('.container-fluid nav.navbar ul li').find('ul').removeAttr('style');                
                ...
                if ( ($(window).height() - $(this).offset().top) < submenu_height ) {
                    ...
                    top_drop = ($(this).offset().top - $(window).scrollTop());

                    if ( $(this).closest('li').find('ul > li').length === 1 ) {
                        top_drop = (top_drop + 60)
                    } else if ( $(this).closest('li').find('ul > li').length === 2 ) {
                        top_drop = (top_drop + 20)
                    }
