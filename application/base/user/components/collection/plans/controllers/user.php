...
class User {
    ...
    public function view() {
        ...
        if ( $this->CI->input->get('p', TRUE) ) {
                    ...
                    if ( is_numeric( $plan_id ) ) {
                        ...
                        if ( $plan ) {
                            ...
                            if ( !empty($plan_data[0]->visible) ) {
                                ...
                                if ( $plan_data[0]->plan_id !== the_user_option( 'plan', $this->CI->user_id ) ) {
                ...
                if ( !empty($complete_transaction['options']) ) {
                    ...
                    if ( $options ) {
                        ...
                        if ( isset($options['plan_id']) ) {
                            ...
                            // Get the user's plan
                            $user_plan = the_user_option( 'plan', $this->CI->user_id );
                                        ...
                                        if ( $referral ) {
                                            ...
                                            if ( $referral[0]->earned < 1 ) {
                                                ...
                                                if ( the_option('referrals_exact_gains') ) {
                                                    ...
                                                    $the_plan_meta = $this->CI->base_model->get_data_where('plans_meta', 'meta_value AS referrals_exact_revenue', array(
                                                    ...
                                                    if ( !empty($the_plan_meta) ) {
                                                        ...
                                                        if ( is_numeric($the_plan_meta[0]['referrals_exact_revenue']) ) {
                                                            ...
                                                            $this->CI->referrals->add_earnings_to_referral($this->CI->user_id, $options['plan_id'], $the_plan_meta[0]['referrals_exact_revenue']);
                                                    ...
                                                    $the_plan_meta = $this->CI->base_model->get_data_where('plans_meta', 'meta_value AS referrals_percentage_revenue', array(
                                                        'plan_id' => $options['plan_id'],
            ...
            // Get the plan's data
            $user_plan_data = $this->CI->base_model->get_data_where(
                'plans',
                '*',
                array(
                    'plan_id' => the_user_option( 'plan', $this->CI->user_id )
                )
            );
