                        ...
                        if ( isset($installation['php']) ) {
                            ...
                            if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['php']) ) {
                                ...
                                if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['php']) === TRUE) {
                                    ...
                                    // Set app's dir
                                    $app_php_dir = $zip->getNameIndex(0);

                                    // First verify if files exists
                                    if ( $zip->numFiles > 0 ) {

                                        // Verify if the app is already installed
                                        if ( is_dir(MIDRUB_BASE_PATH . 'user/apps/collection/' . $app_php_dir) ) {
                                            ...
                                            if ( is_dir(MIDRUB_BASE_PATH . 'user/apps/collection/' . $app_php_dir . '/models/') ) {

                                                // List all available models
                                                foreach ( glob(MIDRUB_BASE_PATH . 'user/apps/collection/' . $app_php_dir . '/models/*.php') as $filename ) {

                                                    // Get the class's name
                                                    $className = str_replace(array(MIDRUB_BASE_PATH . 'user/apps/collection/' . $app_php_dir . '/models/', '.php'), '', $filename);

                                                    // Load the model
                                                    $this->CI->load->ext_model(MIDRUB_BASE_PATH . 'user/apps/collection/' . $app_php_dir . '/models/', ucfirst($className), $className );
                                                    ...
                                                    if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['assets']) ) {
                                                        ...
                                                        if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['assets']) === TRUE) {
                                                            ...
                                                            if ( $zip->numFiles > 0 ) {
                                                                ...
                                                                // Set app's assets
                                                                $app_php_assets = $zip->getNameIndex(0);

                                                                // Verify if the app is already installed
                                                                if ( is_dir(FCPATH . 'assets/base/user/apps/collection/' . $app_php_assets) ) {
