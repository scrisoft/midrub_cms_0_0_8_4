...
set_storage_location (
    ...
    array(
        ...
        'location_upload_from_url' => function ($params) {
            ...
            if ( empty($params['cover_url']) ) {
                ...
                return array(
                    'success' => FALSE,
                    'message' => $CI->lang->line('media_cover_url_missing')
                );
                
            }

            // Verify if the file's url exists
            if ( empty($params['file_url']) ) {

                // Return error response
                return array(
                    'success' => FALSE,
                    'message' => $CI->lang->line('media_file_url_missing')
                );
            ...
            if ( filter_var($params['cover_url'], FILTER_VALIDATE_URL) !== FALSE ) {
                ...
                // Save cover
                file_put_contents(FCPATH . 'assets/share/' . $thumb_name, file_get_contents($params['cover_url']));
                ...
                try {
                    ...
                    // Save the cover
                    file_put_contents(FCPATH . 'assets/share/' . $thumb_name, $params['cover_url']);                
                    ...
                    return array(
                        'success' => FALSE,
                        'message' => $CI->lang->line('media_cover_not_saved')
                    );  
            ...
            // Set read permission
            chmod(FCPATH . 'assets/share/' . $thumb_name, 0644); 

            // Verify if the cover was saved
            if ( !file_exists(FCPATH . 'assets/share/' . $thumb_name) ) {
            ...
            // Save file
            file_put_contents(FCPATH . 'assets/share/' . $file_name . '.' . $params['file_extension'], fopen($params['file_url'], 'r'));

            // Set read permission
            chmod(FCPATH . 'assets/share/' . $file_name . '.' . $params['file_extension'], 0644); 

            // Verify if the file was saved
            if ( !file_exists(FCPATH . 'assets/share/' . $file_name . '.' . $params['file_extension']) ) {

                // Return error response
                return array(
                    'success' => FALSE,
                    'message' => $CI->lang->line('media_file_not_saved')
                );                
            ...
            // Save media in the database
            $last_id = $CI->media->save_media($user_id, base_url('assets/share/' . $file_name . '.' . $params['file_extension']), str_replace(substr(strrchr($params['file_type'], '/'), 0), '', $params['file_type']), base_url('assets/share/' . $thumb_name), $params['file_size'], $params['category']);
            ...
            if ( $last_id ) {
                ...
                return array(
                    'success' => TRUE,
                    'media_id' => $last_id,
                    'media_cover' => base_url('assets/share/' . $thumb_name),
                    'user_storage' => calculate_size($params['total_storage']),
                    'message' => $CI->lang->line('media_file_uploaded_successfully')
                );                           
