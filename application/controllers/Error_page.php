...
class Error_page extends CI_Controller {
    ...
    public function __construct() {
        ...
        if ( isset( $this->session->userdata['username'] ) ) {
            ...
            $this->load->ext_model(MIDRUB_BASE_PATH . 'models/', 'Base_users', 'base_users');

            // Load Base Plans Model
            $this->load->ext_model( MIDRUB_BASE_PATH . 'models/', 'Base_plans', 'base_plans' );            
            ...
            if ( $user_data ) {
                ...
                // Get user language
                $user_lang = the_user_option('user_language');
        ...
        switch ( $type ) {
            ...
            case 'subscription-expired':
                ...
                if ( $this->user_id ) {
                    ...
                    if ( (strtotime(the_user_option( 'plan_end', $this->user_id )) + 86400) > time() ) {
                    ...
                    // Get the user's plan
                    $user_plan = the_user_option( 'plan', $this->user_id );
                    ...
                    // Get the plan
                    $the_plan = $this->base_model->get_data_where(
                        'plans',
                        '*',
                        array(
                            'plan_id' => $user_plan
                    ...
                    if ( $this->input->get('p', TRUE) ) {
                        ...
                        switch ( $this->input->get('p', TRUE) ) {
                            ...
                            case 'order':
                                ...
                                // Get the plan
                                $plan_data = $this->base_model->get_data_where(
                                    'plans',
                                    '*',
                                    array(
                                        'plan_id' => $plan_id
                                ...
                                if ( $plan_data ) {
                                    ...
                                    if ( (int)$plan_data[0]['plan_price'] > 0 ) {
                                        ...
                                        if ( $this->base_plans->change_plan(array(
                                                ...
                                                'plan_end' => the_user_option( 'plan_end', $this->user_id )
                                            )) ) {
                                            ...
                                            $subscriptions = $this->base_model->get_data_where('subscriptions', '*', array(
                                                'user_id' => $this->user_id
                                            ));
                            ...
                            case 'pay':
                                ...
                                if ( $plan_data ) {
                                    ...
                                    // General functions
                                    require_once MIDRUB_BASE_PATH . 'user/components/collection/plans/inc/general.php';
                        ...
                        if ( $this->session->flashdata('complete_transaction') ) {
                            ...
                            if ( isset($transaction['transaction_id']) ) {
                                ...
                                if ( is_numeric($transaction['transaction_id']) ) {
                                    ...
                                    $get_transaction = $this->base_model->get_data_where('transactions', 'transactions.*, users.username', array('transactions.transaction_id' => $transaction['transaction_id'], 'transactions.user_id' => $this->user_id),
                                        array(),
                                        array(),
                                        array(array(
                                            'table' => 'users',
                                    ...
                                    if ( $get_transaction ) {
                                        ...
                                        $fields = $this->base_model->get_data_where('transactions_fields', '*', array('transaction_id' => $transaction['transaction_id']));
                                        ...
                                        $options = $this->base_model->get_data_where('transactions_options', '*', array('transaction_id' => $transaction['transaction_id']));
                                        ...
                                        if ( !empty($transaction['options']) ) {
                                            ...
                                            if ( $options ) {
                                                                ...
                                                                if ( the_option('enable_referral') ) {
                                                                    ...
                                                                    if ( $referral ) {
                                                                        ...
                                                                        if ( $referral[0]->earned < 1 ) {
                                                                            ...
                                                                            if ( the_option('referrals_exact_gains') ) {

                                                                                // Get the referrals_exact_revenue value
                                                                                $the_plan_meta = $this->base_model->get_data_where('plans_meta', 'meta_value AS referrals_exact_revenue', array(
                                                                                    'plan_id' => $options['plan_id'],
                                                                                ...
                                                                                $the_plan_meta = $this->base_model->get_data_where('plans_meta', 'meta_value AS referrals_percentage_revenue', array(
                                                                                    'plan_id' => $options['plan_id'],
                                                ...
                                                if ( isset($options['discount_code']) ) {
                                                    ...
                                                    // Search for coupon
                                                    $coupon = $this->base_model->get_data_where('coupons', '*', array(
